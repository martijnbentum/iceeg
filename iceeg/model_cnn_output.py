...
class model_cnn_output:
	...
	def __init__(self,data,batch_size = 200,nsamples= 201,perc_artifact= .93,gradient_descent_opt= 0.01,tf_version = '1.2.1'):
	...
	def train(self,ncycles= 100000):
		...
		for i in range(ncycles):
			self.bar.update(i)
			batch = next_batch_ratio(self.data.artifact_train,self.data.clean_train,self.perc_artifact,self.batch_size)
			if i % 100000 == 0:
...
def next_batch_ratio(artifact_data ,clean_data ,perc_artifacts =.93, batch_size= 200):
	'''Create a batch of nsamples randomly drawn rows with a ratio clean/artifact 
	defined by perc_artifact.'''
	nartifact, nclean = samplen_artifact_clean(batch_size,perc_artifacts)
