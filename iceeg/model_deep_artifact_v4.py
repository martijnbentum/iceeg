...
class model_deep_artifact:
	...
	def __init__(self,info = None,data = None,batch_size = 200,nchannels=24,nsamples= 100,test_set_perc= 0.01,tf_version = '1.2.1',model_name = ''):
	...
	def make_model_name(self):
		...
		if os.path.isfile(path.model + self.model_name + '_' + str(self.model_save_counter)):

			
		

	def save_model(self,model_name = 'None'):
		if model_name == None: model_name = self.make_model_name()
		if not hasattr(self,'saver'):
			self.saver = tf.train.Saver()
		self.saver(self.sess,model_name)
...
def deepnn(x,nchannels,nsamples,fmap_size = 2400):
	...
	with tf.name_scope('fc1'):
		...
		h_pool2_flat = tf.reshape(h_pool2, [-1,6 * 25* 128],name = 'h_pool3_flat-bla')
