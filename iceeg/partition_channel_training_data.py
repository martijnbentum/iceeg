...
def make_xfold(total_nrows, x = 100):
...
def save_fold_indices(i_folds):
	...
	for i,f in enumerate(i_folds):
		...
		np.save(path.channel_artifact_training_data + 'PART_INDICES/indices_part-'+str(i+1),f)
	...
	for i,line in enumerate(datafn_all_rows_before):
		if i < len(datafn_all_rows_before) -1:
			if datafn_all_rows_before[i+1][1] > all_index >= line[1]:
				...
				return line
		elif  17124221 > all_index >= line[1]:
...
def make_part_file(filenames_and_indices):
	...
	data_output = np.zeros((nrows,2600))
	info_output = np.zeros((nrows,26))
	start_index = 0
	for i,f in enumerate(filenames_and_indices):
		print(f,i,len(filenames_and_indices))
		d = np.load(path.channel_artifact_training_data + f.replace('info','data'))
		info = np.load(path.channel_artifact_training_data + f)
