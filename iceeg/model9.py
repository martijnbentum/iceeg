...
'''
first conv with 2X1 for each duo of bad/ good channel and subsequently a maxpool
to cut down input 52X100 -> 26X100
training results were not encouraging
'''
...
def conv2d_alt(x, W):
	"""conv2d returns a 2d convolution layer no overlap stride."""
...
def deepnn(x,nchannels,nsamples,kernel_size,fmap_size = 2400):
	...
	# zeroth convolutional layer - maps bad/good channel duo's to 8 feature maps and halves 
	# nchannel dimension.
	with tf.name_scope('conv0'):
		W_conv0 = weight_variable([kernel_size, 1, 1, 4])
		b_conv0 = bias_variable([4])
		h_conv0 = tf.nn.relu(conv2d(x_image, W_conv0) + b_conv0)
		print(W_conv0,'W_conv0')
		print(b_conv0,'b_conv0')
		print(h_conv0,'h_conv0')
	
	# Pooling layer - downsamples by 2X.
	with tf.name_scope('pool0'):
		h_pool0 = max_pool_2vertical(h_conv0)
		print(h_pool0,'h_pool0')

	# First convolutional layer - maps one grayscale image to 32 feature maps.
	with tf.name_scope('conv1'):
		W_conv1 = weight_variable([1, 25, 4, 32])
	...
	with tf.name_scope('fc1'):
		height = calc_height(nchannels,3)
...
def calc_height(nchannels,n_steps):
	if n_steps > 1:
		return calc_height(math.ceil(nchannels/2), n_steps = n_steps -1)
	else:
		return math.ceil(nchannels/2)
