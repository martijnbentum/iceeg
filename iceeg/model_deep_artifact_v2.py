...
def deepnn(x,nchannels,nsamples,fmap_size = 1248):
	...
	with tf.name_scope('fc2'):
		W_fc2 = weight_variable([fmap_size, 600],name='W_fc2-bla')
		b_fc2 = bias_variable([600],name='b_fc2-bla')
		print(W_fc2,'W_fc2')
		print(b_fc2,'b_fc2')
		h_fc2 = tf.nn.relu(tf.matmul(h_fc1, W_fc2) + b_fc2,name='h_fc1-bla')

	# Dropout - controls the complexity of the model, prevents co-adaptation of
	# features.
	with tf.name_scope('dropout'):
		keep_prob = tf.placeholder(tf.float32,name = 'keep_prob-bla')
		h_fc2_drop = tf.nn.dropout(h_fc2, keep_prob,name = 'h_fc2_drop-bla')

	# Map the final features to 2 classes, one for each digit
	with tf.name_scope('fc3'):
		W_fc3 = weight_variable([600, 2],name='W_fc3-bla')
		b_fc3 = bias_variable([2],name='b_fc3-bla')
		print(W_fc3,'W_fc3')
		print(b_fc3,'b_fc3')
