...
class scheduler:
	'''Create an object that holds every which performs a task every n units given certain conditions.
...
class every:
	'''Create an object that performs a task every n units given certain conditions.
	The task is performed in a seperate thread.
	'''
	def __init__(self,interval = 1, unit = 'second',conditions = [], function = None,name='default',args = (), kwargs = {},maximum_nexecuters=10):
		...
		conditions 		the conditions when the function is executed every n units, ie weekdays
		...
		kwargs 			keyword argument dictionary to be passed to the function
		maximum_... 	if the task is longer than the interval, this will cap the number of executers
		'''
	...
	def check_conditions(self):
		'''Check whether the conditions are true (if conditions every performs task every n units).'''
		if len(self.conditions) > 2: print('probably to many conditions',self.conditions)
...
def h2s(h):
	'''transform hours to seconds. some special function I do not understand'''
	if len(str(h)) > 2: return int(h/100) * 3600 + h%100 * 60
	else: return h * 60 

def s2h(s):
	'''Transform seconds to hours. some special function I do not understand'''
	h = int(s/3600) * 100
	s = s%3600
	h += int(s/60)
	return h
