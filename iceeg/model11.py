...
'''
adapted from model7
could be usefull for very small k 2 and 3
add another layer to reduce dimensions
if k = 2 input = 52X100, if k =3 input = 39X100
in model7 this is reduce to 13 and 10
while with k= 6 the output is reduced to 8
'''
...
def deepnn(x,nchannels,nsamples,kernel_size,fmap_size = 2400):
	...
	with tf.name_scope('fc1'):
		...
		W_fc1 = weight_variable([height * 13 * 256, fmap_size])
		b_fc1 = bias_variable([fmap_size])
		print(W_fc1,'W_fc1')
		print(b_fc1,'b_fc1')

		h_pool2_flat = tf.reshape(h_pool3, [-1, height* 13* 256],name = 'h_pool2_flat-bla')
